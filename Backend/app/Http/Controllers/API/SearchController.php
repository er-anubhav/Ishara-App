...
class SearchController extends Controller
{
    ...
    public function search(Request $request){
        ...
        $hasFileCategoryColumn = Schema::hasColumn('files', 'category');
        ...
        if ($hasRoleColumn) {
            $doctors = $doctors->addSelect('id', 'name', 'role as tag', 'specializations', 'experience_years', 'experience_months');
        } else {
            $doctors = $doctors->addSelect('id', 'name', DB::raw("'Doctor' as tag"), 'specializations', 'experience_years', 'experience_months');
        }
        $doctors = $doctors->inRandomOrder()->get();
        ...
        if ($hasFileCategoryColumn) {
            $files = $files->addSelect('id', 'name', 'category', 'file_type', 'created_at', DB::raw("'file' as tag"));
        } else {
            $files = $files->addSelect('id', 'name', DB::raw('NULL as category'), 'file_type', 'created_at', DB::raw("'file' as tag"));
        }
        $files = $files->get();
    ...
    public function files_and_folders(Request $request)
    {
        ...
        if($profile_id){
            ...
            //Filters
            if ($request->category && $hasFileCategoryColumn) {
            ...
            if ($hasFileCategoryColumn) {
                ...
                $files = $files->inRandomOrder()->limit(5)->addSelect('id', 'name', DB::raw('NULL as category'), 'file_type', 'created_at', DB::raw("'file' as tag"))->get();
            }
