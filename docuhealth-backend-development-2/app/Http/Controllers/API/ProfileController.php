...
class ProfileController extends Controller
{
    ...
    public function profiles()
    {
        ...
        $data['user'] = $user;
        $data['profiles'] = $this->beautify->userProfiles($profiles);
    ...
    public function my_profile(Request $request)
    {
        if ($profile = UserProfile::find($request->profile->id)) {
            ...
            $data['user'] = $user;
            $data['profile'] = $this->beautify->profileOBJ($profile);
            return response()->json($this->structure(true, 'Success', $data), 200);
        }
    ...
    public function update_current_location(Request $request)
    {
        ...
        if ($request->location){
            ...
            $data['full_address'] = $request->location;
            $data['longitude'] = $request->longitude;
            $data['latitude'] = $request->latitude;

            $user->location = json_encode($data, true);
        }

        if ($user->save()) {
            return response()->json($this->structure(true, "Location Updated successfully.", [$user]), 200);
        }
        return response()->json($this->structure(false, 'Somthing went wrong!'), 200);
    }
}
