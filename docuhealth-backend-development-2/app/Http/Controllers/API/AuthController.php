    ...
    public function verify(Request $request)
    {
        ...
        if (!$validator->fails()) {
            ...
            if ($user->save()) {
                ...
                $action = empty($user->first_name) ? 'go_to_register' : 'go_to_profiles';

                return response()->json($this->action_with_token_structure(true, 'User logged in successfully', [$user], $token, $action), 200);
            }
            return response()->json($this->token_structure(false, 'Somthing Went Wrong!'), 200);
        } 
        ...
        if (!$validator->fails()) {
            ...
            if ($user = User::find($id)) {
                ...
                $profile = UserProfile::where('id', $request->profile_id)
                                    ->exclude(['created_at', 'updated_at', 'deleted_at', 'icon'])->first();
                if ($profile) {

                    if ($profile->user_id == $user->id || $profile->parent_id == $user->id) {
                        
                        $token = auth('api')->claims(['logged_in_profile' => $profile])->login($user);
                        return response()->json($this->token_structure(false, $profile->name.' Login Success.', $user, $token), 200);
                    }
                }
            }
